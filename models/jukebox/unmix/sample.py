...
from unmix.data.labels import EmptyLabeller
from unmix.utils.torch_utils import empty_cache
from unmix.utils.audio_utils import save_wav, load_audio
from unmix.make_models import make_model
from unmix.align import get_alignment
from unmix.save_html import save_html
from unmix.utils.sample_utils import split_batch, get_starts
from unmix.utils.dist_utils import print_once
import fire
...
def sample_level(zs, labels, sampling_kwargs, level, prior, total_length, hop_length, hps):
    ...
    if total_length >= prior.n_ctx:
        for start in get_starts(total_length, prior.n_ctx, hop_length):
            zs = sample_single_window(
                zs, labels, sampling_kwargs, level, prior, start, hps)
    else:
...
def _sample(zs, labels, sampling_kwargs, priors, sample_levels, hps):
    ...
    for level in reversed(sample_levels):
        ...
        zs = sample_level(zs, labels[level], sampling_kwargs[level],
                          level, prior, total_length, hop_length, hps)
        ...
        if not os.path.exists(logdir):
            os.makedirs(logdir)
        t.save(dict(zs=zs, labels=labels, sampling_kwargs=sampling_kwargs,
        ...
        if alignments is None and priors[-1] is not None and priors[-1].n_tokens > 0 and not isinstance(priors[-1].labeller, EmptyLabeller):
            alignments = get_alignment(
                x, zs, labels[-1], priors[-1], sampling_kwargs[-1]['fp16'], hps)
...
def primed_sample(x, labels, sampling_kwargs, priors, hps):
    sample_levels = list(range(len(priors)))
    zs = priors[-1].encode(x, start_level=0,
                           end_level=len(priors), bs_chunks=x.shape[0])
    zs = _sample(zs, labels, sampling_kwargs, priors, sample_levels, hps)
    return zs
...
def load_prompts(audio_files, duration, hps):
    ...
    for audio_file in audio_files:
        x = load_audio(audio_file, sr=hps.sr,
                       duration=duration, offset=0.0, mono=True)
        x = x.T  # CT -> TC
        xs.append(x)
...
def load_codes(codes_file, duration, priors, hps):
    ...
    if duration is not None:
        ...
        zs = [z[:, :duration//prior.raw_to_tokens]
              for z, prior in zip(zs, priors)]
    return zs
...
def save_samples(model, device, hps, sample_hps):
    print(hps)
    from unmix.lyricdict import poems, gpt_2_lyrics
    ...
    labels = [prior.labeller.get_batch_labels(
        metas, 'cuda') for prior in priors]
    for label in labels:
        assert label['y'].shape[0] == hps.n_samples

    lower_level_chunk_size = 32
    lower_level_max_batch_size = 10
    ...
    sampling_kwargs = [dict(temp=0.99, fp16=True, chunk_size=lower_level_chunk_size, max_batch_size=lower_level_max_batch_size),
                       dict(temp=0.99, fp16=True, chunk_size=lower_level_chunk_size,
    ...
    if sample_hps.mode == 'ancestral':
        ...
        if sample_hps.prompt_length_in_seconds is not None:
            duration = (int(sample_hps.prompt_length_in_seconds *
                        hps.sr) // top_raw_to_tokens) * top_raw_to_tokens
        else:
            duration = None
...
def run(model, mode='ancestral', codes_file=None, audio_file=None, prompt_length_in_seconds=None, port=29500, **kwargs):
    ...
    sample_hps = Hyperparams(dict(mode=mode, codes_file=codes_file,
                             audio_file=audio_file, prompt_length_in_seconds=prompt_length_in_seconds))
