...
def _loss_fn(loss_fn, x_target, x_pred, hps):
     # NEW TEST
    def _sdr_loss(x_target, x_pred):
        # Assuming x_target and x_pred are both non-zero signals
        # SDR is usually calculated in decibels (dB)
        # SDR = 10 * log10(||x_target||^2 / ||x_target - x_pred||^2)
        ratio = t.norm(x_target, p=2) ** 2 / (t.norm(x_target - x_pred, p=2) ** 2 + 1e-8)
        sdr = 10 * t.log10(ratio + 1e-8)  # Adding a small epsilon to prevent division by zero
        return -sdr  # Negate since we want to maximize SDR, but optimizers minimize the loss
    # END NEW
    if loss_fn == 'l1':
        ...
        #NEW: ADD SDR
        sdr_weight = 0.1  # For example, SDR loss has 10% weight of the total loss
        
        sdr_loss = _sdr_loss(x_target, x_pred)
        print(f"unweighted sdr loss: {sdr_loss}")
        loss += sdr_weight * sdr_loss
...
class VQVAE(nn.Module):
    ...
    #NEW: ADDED Y HERE
    def forward(self, x, y, hps, loss_fn='l1'):
        metrics = {}
        ...
        #NEW: Change from x.float() to y.float()
        x_target = audio_postprocess(y.float(), hps)
