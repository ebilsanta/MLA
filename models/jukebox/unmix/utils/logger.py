...
class Logger:
    ...
    #NEW: Account for 2 loaders
    def get_range(self, loader1, loader2):
        # Combine the two loaders
        combined_loader = zip(loader1, loader2)

        if self.rank == 0:
            # Use tqdm for the first rank for a progress bar
            self.trange = def_tqdm(combined_loader)
        else:
            # For other ranks, do not use the progress bar
            self.trange = combined_loader
