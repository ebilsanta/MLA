...
def evaluate(model, orig_model, logger, metrics, data_processor, hps):
    ...
    #NEW: Haiz please work :(
    with t.no_grad():
        # Modified to use zip to iterate over both test loaders
        for i, (original_data, stem_data) in logger.get_range(data_processor.test_loader, data_processor.stem_test_loader):
            # Unpack data if they're in tuples/lists
            x = original_data[0] if isinstance(original_data, (tuple, list)) else original_data
            y = stem_data[0] if isinstance(stem_data, (tuple, list)) else stem_data

            x = x.to('cuda', non_blocking=True)
            y = y.to('cuda', non_blocking=True)

            x_in = audio_preprocess(x, hps)
            y_in = audio_preprocess(y, hps)  # Assuming same preprocessing can be applied to y

            log_input_output = (i == 0)

            if hps.prior: #dk if will bug out here but i dont think i'm using the prior, so it should be ok
                forw_kwargs = dict(y=y_in, fp16=hps.fp16, decode=log_input_output)
            else:
                forw_kwargs = dict(loss_fn=hps.loss_fn, hps=hps)

            # Forward pass includes both x and y
            x_out, loss, _metrics = model(x_in, y_in, **forw_kwargs)
    #END NEW
            ...
            with t.no_grad():
                ...
                #NEW:
                    log_inputs(orig_model, logger, x_in, y_in, x_out, hps)
...
def train(model, orig_model, opt, shd, scalar, ema, logger, metrics, data_processor, hps):
    ...
    # NEW: Loop through both x & y
    for i, (original_data, stem_data) in logger.get_range(data_processor.train_loader, data_processor.stem_train_loader):
