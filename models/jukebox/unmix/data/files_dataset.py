...
from unmix.utils.io import get_duration_sec, load_audio
from unmix.data.labels import Labeller


class FilesAudioDataset(Dataset):
    #NEW: added directory=None
    def __init__(self, hps, directory=None):
        super().__init__() 
        #NEW: added directory
        self.directory = directory if directory is not None else hps.audio_files_dir
        self.sr = hps.sr
    ...
    #New: removed taking in hps (self, hps)
    def init_dataset(self, hps):
        # Load list of files and starts/durations
        #NEW: change to self.directory
        files = librosa.util.find_files(f'{self.directory}', [
        ...
        if self.labels:
            self.labeller = Labeller(
                hps.max_bow_genre_size, hps.n_tokens, self.sample_length, v3=hps.labels_v3)
    ...
    def get_song_chunk(self, index, offset, test=False):
        ...
        if self.labels:
            ...
            labels = self.labeller.get_label(
                artist, genre, lyrics, total_length, offset)
