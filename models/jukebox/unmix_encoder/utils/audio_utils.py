...
import soundfile
import librosa
from unmix_encoder.utils.dist_utils import print_once
...
def calculate_bandwidth(dataset, hps, duration=600):
    ...
    while n_seen < n_samples and idx<len(dataset):
        ...
        stft = librosa.core.stft(np.mean(
            samples, axis=1), hps.n_fft, hop_length=hps.hop_length, win_length=hps.window_size)
    ...
    if dist.is_available():
        from unmix_encoder.utils.dist_utils import allreduce
...
def multispectral_loss(x_in, x_out, hps):
    losses = []
    assert len(hps.multispec_loss_n_fft) == len(
        hps.multispec_loss_hop_length) == len(hps.multispec_loss_window_size)
...
def load_audio(file, sr, offset, duration, mono=False):
    ...
    x, _ = librosa.load(file, sr=sr, mono=mono,
                        offset=offset/sr, duration=duration/sr)
    if len(x.shape) == 1:
        x = x.reshape((1, -1))
    return x
...
def save_wav(fname, aud, sr):
    ...
    for i in list(range(aud.shape[0])):
        soundfile.write(f'{fname}/item_{i}.wav',
                        aud[i], samplerate=sr, format='wav')
