...
parser.add_argument('--encoder', help='encoder',
                    required=False, type=int, default=60001)
parser.add_argument("--freeze", help="jukebox/trained: means no training/ training",
                    action="store", dest="freeze", default='default')

args = parser.parse_args()

#python predict_channel.py --id 1 --vqvae [step name will be used for restoring] --encoder [also to restore encoder] --freeze [jukebox, if necessary]
...
#TODO: Edit audio file paths + hyperparameters here!

#just testing
if file_des == "jukebox":
    print("Jukebox")
    hps = setup_hparams("vqvae", dict(name="vqvae_"+channel_name+"_predict", sr=44100, sample_length=393216, bs=6, audio_files_dir="new_data/1mixtest/",
                                      labels=False, train=False, aug_shift=True, aug_blend=True, restore_vqvae='logs/vqvae_bass_b6/checkpoint_step_80001.pth.tar'))

# if file_des == "jukebox":
#     print("Jukebox")
#     hps = setup_hparams("vqvae", dict(name="vqvae_"+channel_name+"_predict", sr=44100, sample_length=393216, bs=6, audio_files_dir="unmix/data/test/mixture/",
#                                       labels=False, train=False, aug_shift=True, aug_blend=True, restore_vqvae=REMOTE_PREFIX + 'jukebox/models/5b/vqvae.pth.tar'))  # restore_vqvae="/media/compute/homes/wzaielamri/ai_music/unmix/logs/vqvae_rest_b4/checkpoint_step_100001.pth.tar"))
    ...
    print("Trained")
    hps = setup_hparams("vqvae", dict(name="vqvae_"+channel_name+"_predict", sr=44100, sample_length=393216, bs=6, audio_files_dir="new_data/1mixtest/",
                                      labels=False, train=False, aug_shift=True, aug_blend=True, restore_vqvae="logs/vqvae_bass_b6/checkpoint_step_80001.pth.tar"))  # restore_vqvae="/media/compute/homes/wzaielamri/ai_music/unmix/logs/vqvae_rest_b4/checkpoint_step_100001.pth.tar"))
    #restore_vqvae="logs/vqvae_"+channel_name+"_b4/checkpoint_step_"+vqvae_step+".pth.tar"
    # restore_vqvae=REMOTE_PREFIX + 'jukebox/models/5b/vqvae.pth.tar'))

#TODO: Edit path here! 

# encoder_cp = "logs/unmix_encoder/encoder_" + \
#     channel_name+"_b4/checkpoint_step_"+encoder_step+".pth.tar"

# encoder_cp = "logs/encoder_bass_b6/checkpoint_step_80001.pth.tar"
...
for ind, audio_file in enumerate(audio_files):
    ...
    for j in range(0, chunks):
        ...
        #NEW: pass in anoter x as a dummy y
        ...
        #Start of new code
        results_dir = "results_" + channel_name
        ensure_directory_exists(results_dir)  # Ensure the directory exists
        audio_name = audio_file.split('/')[-1]

        #TODO: Edit change here!
        np.save(os.path.join(results_dir, file_des+"_" + 
    ...
    start_time = time.time()
    #NEW: pass in another x as dummy y
    x_out, loss, _metrics = vqvae(x, x, **forw_kwargs)
    ...
    #TODO: Edit change here!
    audio_name = audio_file.split('/')[-1]
    np.save(os.path.join(results_dir, file_des+"_" + audio_name + "_" + str(chunks) + ".npy"), x_out)
    # end of new code
